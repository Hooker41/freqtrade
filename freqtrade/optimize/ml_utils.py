...
def run_pipeline(data, test_ratio):
    """Main util function that runs full pipeline.
       ...
       and itmust not be in the coins_list.
    """
    out_coin_prices = data

    X_train, y_train, X_test, y_test = PreprocessData(data, test_ratio)

    parameters = []
    savemodel = False
    (accuracy, predictions) = performGTBClass(X_train, y_train, X_test, y_test, parameters, savemodel)
    print ("Prediction accuracy on test set: ", accuracy)
    # Predictions is pd series of predicted Up/downs indexed by date.
    output = pd.concat([out_coin_prices, predictions], axis = 1)
    output['Prediction'] = output['Prediction'].shift(1)
    # NaN values correspond to training set, they should be dropped.
    output = output.dropna()
    return output
...
def PreprocessData(df, test_ratio):
    ...
    X_train = X[2:num_testing_points]
    y_train = y[2:num_testing_points]
    X_test = X[num_testing_points:]
    y_test = y[num_testing_points:]
...
def performGTBClass(X_train, y_train, X_test, y_test, parameters, savemodel):
    ...
    prediction_indexed['Prediction'] = pd.Series(prediction, index = X_test.index)
